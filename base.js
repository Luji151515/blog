...
//设置物体居中
Base.prototype.center = function (x,y) {
	for (var i = 0; i < this.elements.length; i ++) {
		if(x&&y){
			var top = (getInner().height - this.elements[i].offsetWidth)/2+(getScroll().top);
			var left = (getInner().width - this.elements[i].offsetHeight) /2+(getScroll().left);
		}else if(x&&!y){
			var top = (getInner().height - this.elements[i].offsetWidth)/2;
			var left = (getInner().width - this.elements[i].offsetHeight) /2+(getScroll().left);
		}else if(!x&&y){
            var top = (getInner().height - this.elements[i].offsetWidth)/2+(getScroll().top);
			var left = (getInner().width - this.elements[i].offsetHeight) /2;
		}else{
			var top = (getInner().height - this.elements[i].offsetWidth)/2;
			var left = (getInner().width - this.elements[i].offsetHeight) /2;
		}
			...
			*/
			...
			for (var i in mul) {
				...
				if (attr == 'opacity') {
					if (step == 0) {
						setOpacity();
					} else if (step > 0 && Math.abs(parseInt(getStyle(element, attr) * 100 - target)) <= step) {
						setOpacity();
					} else if (step < 0 && (parseInt(getStyle(element, attr) * 100 - target) <= Math.abs(step))) {
						setOpacity();
					} else {
